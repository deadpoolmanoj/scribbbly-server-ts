...
const const_values_1 = require("./utils/const-values");
...
app.post('/room', (req, res) => {
    const roomId = (0, helper_functions_1.generateRoomId)();
    rooms[roomId] = {
        id: roomId,
        currentRound: 1,
        gameStarted: false,
        ...
        maxRounds: const_values_1.DEFAULT_ROUNDS,
        timeLeft: const_values_1.DEFAULT_ROUND_TIME,
        drawTime: const_values_1.DEFAULT_ROUND_TIME,
        selectedLanguage: const_values_1.DEFAULT_LANGUAGE,
        wordSelectionTime: const_values_1.DEFAULT_WORD_SELECTION_TIME,
        pointsBoardTime: 2
    };
    res.json({ roomId });
});
...
function getUniqueBrainrotName(room) {
    let randomName = (0, helper_functions_1.getRandomBrainrotName)();
    const existingNames = room.players.map(player => player.name);
    while (existingNames.includes(randomName)) {
        randomName = (0, helper_functions_1.getRandomBrainrotName)();
    }
    return randomName;
}
...
io.on('connection', (socket) => {
    ...
    socket.on('settings-change', ({ roomId, roomProp, value }) => {
        ...
        room.messages?.push({
            text: `${reverseSettingKeyMap[roomProp]}  SET TO ${value}`,
            type: 'system',
        });
    ...
    function wordSelectionCountDown(room) {
        ...
        }, const_values_1.COUNTDOUN_UNIT);
    }
    function drawingCountDown(room) {
    ...
    function showPointsBoard(room) {
        const pointsBoardInterval = setInterval(() => {
            ...
            if (room.pointsBoardTime <= 0) {
                ...
                room.wordSelectionTime = const_values_1.DEFAULT_WORD_SELECTION_TIME;
                room.timeLeft = const_values_1.DEFAULT_ROUND_TIME;
    ...
    function nextTurn(roomId) {
        ...
        room.currentWord = (0, helper_functions_1.getRandomDrawableWord)();
        room.currentDrawerId = room.turnOrder[nextIndex];
